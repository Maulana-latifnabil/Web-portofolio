...
function Work({ position, company, location, type, duration, image }) {
  return (
    <article className="pt-8 border-b-2 border-dark-content pb-5 dark:border-light-content border-opacity-20 dark:border-opacity-20">
      ...
      {image && Array.isArray(image) && image.length > 0 && (
        <div className="my-4 flex overflow-x-auto">
          {image.map((img, index) => (
            <div
              key={index}
              className="m-2 p-1"
            ...
            >
              <img
                ...
                className="object-cover border border-gray-300 rounded-md w-full h-48"
                loading="lazy"
